...
class KDAKernelDispatcher:
    ...
    def __init__(
        ...
        # The speculative target-verify kernel is selected independently of the
        # decode / prefill kernels: only Triton and cuLA implement target_verify.
        if verify_backend.is_triton():
            self.verify_kernel = triton_kernel
        elif verify_backend.is_any_cula():
            if not is_cuda():
    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        if is_target_verify:
            ...
            # cuLA ReplaySSM (ring-free) verify scratch; empty for the recurrent /
            # triton verify paths (which use intermediate_ssm instead), so the
            # extra kwargs never reach a kernel that does not expect them.
            replayssm_scratch = {}
